...
const imageSemantics: string[] = ['DOCHOLDER', 'TxtFile', 'ObjName'];
...
const search_options: SearchSettings = {
    'map': { 'id': 'GISWebServiceSE', 'visible': 1 },
    'address': {
        'visible': 1,
        'default': 0,
        'sources': [
            { 'type': AddressServiceType.PanoramaAddressBase, 'alias': 'Адресная база', 'url': 'https://address.gisserver.info/address/fias/address_fias.php', 'result': 100 },
            { 'type': AddressServiceType.Osm, 'alias': 'Osm', 'url': 'https://nominatim.openstreetmap.org/search', 'result': 50 },
            { 'type': AddressServiceType.Yandex, 'alias': 'Yandex', 'url': 'https://geocode-maps.yandex.ru/1.x/', 'result': 50 }
        ]
    },
...
const cartogram: UserThematicOptions = {
    source: [{
...
const copyright: CopyRightSettings = {
    name: 'Panorama',
    startYear: '1991',
    privacyPolicy: {
...
const externalFunctions: ExternalFunctions = [{ description: 'Запросить информацию из БД', name: 'userFunction_1' }, { description: 'Открыть пользовательский компонент', name: 'userFunction_2' }];
...
const gwtkOptions: GwtkOptions = {
    ...
    'routecontrol': [
        {url: 'https://gisserver.info/GISWebServiceSE/service.php', layer: 'rusgraph', type: SourceRoutes.Panorama, alias: 'Маршруты', authtype: ''},
        {url: 'https://api-maps.yandex.ru/2.1/?lang=ru_RU', layer: '', type: SourceRoutes.Yandex, alias: 'Яндекс.Маршруты', authtype: ''}
    ],
    'controls': ['*'],
    search_options,
    contenttree,
...
(window as any).userFunction_1 = (mapObject: MapObject) => {
...
(window as any).userFunction_2 = (mapObject: MapObject) => {
