 ...
 */
export default class AreaTooltipRenderable extends HTMLTooltipRenderable {

    protected createNode( renderer: HTMLrenderer, mapObject: MapObject, style?: Style ): HTMLElement | undefined {
        const value = mapObject.getSemantic( 'area' )?.value || '';
        return AreaTooltipRenderable.createTooltipHtmlElement( renderer, mapObject, value );
    }
     ...
     */
    private static createTooltipHtmlElement( renderer: HTMLrenderer, mapObject: MapObject, value?: string ): HTMLDivElement | undefined {

        if ( !value ) {
            return;
        }

        const point = mapObject.getPoint( {} );
        ...
        if ( point ) {
            ...
            //Положение на экране
            const resultPoint = mapObject.vectorLayer.map.planeToPixel( point );
