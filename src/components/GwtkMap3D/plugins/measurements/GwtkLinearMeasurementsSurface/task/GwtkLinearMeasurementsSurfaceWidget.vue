    ...
    >
        <v-col class="gwtk-measurement-item-list pt-0">
            ...
            <v-list
                ...
                class="py-0"
            >
                <template v-for="(item, index) in measurementListFiltered">
                    <gwtk-list-item
                        :key="index+item.name"
                    ...
                    <v-divider
                        v-if="index !== measurementListFiltered.length-1"
                        :key="index"
                    />
                </template>
            </v-list>
            <v-col v-if="measurementList.length" cols="6" class="pt-0">
                <v-combobox
                    :value="angleTextCurrent"
                    :items="angleTypes"
                    :label="$t('phrases.Angle type')"
                    :item-text="'text'"
                    outlined
                    dense
                    hide-details
                    @change="value => changeAngleType(value)"
                />
            </v-col>
...
<script lang="ts" src="./GwtkLinearMeasurementsSurfaceWidget.ts" />
